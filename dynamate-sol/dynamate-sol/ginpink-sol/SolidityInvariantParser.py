        ...
        try:
            try:
                ...
                if self._state.backtracking == 0:
                    ...
                    # ANTLR3's Python target binds a (...)* label to the
                    # last match, not a list, so iterating it raises. Accept
                    # either shape.
                    _rs = r if isinstance(r, (list, tuple)) else ([r] if r is not None else [])
                    r_exprs = [x.result for x in _rs if x is not None]
