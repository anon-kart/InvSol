...
def generate_decls(trace_runs: list[TraceRun]) -> tuple[str, dict]:
    ...
    for ppt_name in sorted(ppt_declared_vars.keys()):
        ...
        # Daikon numbers exit points so several returns can be told apart, so
        # the suffix is EXIT followed by an optional number.
        if ppt_name.endswith(":::ENTER"):
            lines.append("ppt-type enter")
        elif re.search(r":::EXIT\d*$", ppt_name):
            lines.append("ppt-type exit")
        else:
            raise ValueError(f"unrecognised program point suffix: {ppt_name}")

        # Always include synthetic variables
        # The declared type of `this` is the contract under analysis, which the
        # program point name already carries as its first component.
        enclosing_contract = ppt_name.split(".", 1)[0] or "Contract"
        lines.append("variable this")
        lines.append("  var-kind variable")
        lines.append(f"  dec-type {enclosing_contract}")
        ...
        for var_name, var_type in sorted(ppt_declared_vars[ppt_name].items()):
            if ".toString" in var_name:
                ...
                if var_type == "int":
                    ...
                    # A rep-type of hashcode is a reference, so the declared
                    # type has to be a class name rather than a string.
                    lines.append("  dec-type address")
                    lines.append("  rep-type hashcode")
                else:
                    lines.append("  dec-type java.lang.String")
                    lines.append("  rep-type java.lang.String")
