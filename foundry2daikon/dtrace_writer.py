...
def sanitize_numeric(val: str) -> str:
    """
    Ensures numeric-looking values are valid integers/floats for Daikon.
    If not numeric, replace with 0 to prevent Daikon parse errors.
    """
    s = str(val).strip()
    # Remove stray colons or contract labels like 'LoopPlayground:'
    s = re.sub(r'[^0-9eE\+\-\.]', '', s)
    if s == "" or s in {".", "-", "+"}:
        s = "0"
    return s
        ...
        for ppt_idx, ppt in enumerate(run.program_points):
            ...
            for var_name in sorted(declared_vars.keys()):
                ...
                if var_name in seen_vars:
                    val = str(seen_vars[var_name].value).strip()

                    # Fix known bad patterns like 'LoopPlayground:'
                    if val.endswith(":"):
                    ...
                    if var_type == "hashcode":
                        ...
                        formatted_val = sanitize_numeric(val)
                ...
                # Each variable ends with '1' line (value count)
                lines.append("1")
