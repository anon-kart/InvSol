...
def _harness_pragma(model: Dict) -> str:
    """
    Choose a pragma for the harness that can be satisfied together with the
    contract under test.

    The contract's own constraint is reused when it is known. Widening it
    independently does not work: a contract pinned to =0.8.19 and a harness
    asking for ^0.8.26 have no common compiler, which is a compile error before
    any test runs.
    """
    declared = str(model.get("pragma") or "").strip()
    if declared:
        return f"pragma solidity {declared};"

    version = str(model.get("solidity_version") or "").strip()
    if version:
        return f"pragma solidity >={version} <0.9.0;"

    return "pragma solidity >=0.8.0 <0.9.0;"
...
def generate_harness(model: Dict, cfg: Dict) -> str:
    ...
    L.append(_harness_pragma(model))
    L.append("")
    ...
    for fn in functions:
        name = fn.get("name")
        if not name or name in ("constructor", "receive", "fallback"):
            continue
