    ...
    for lp in loops:
        ...
        body = sig.get("body_summary") or {}
            ...
            IRLoopSignature(
                ...
                node_id=lp.get("node_id"),
                parent_node_id=lp.get("parent_node_id"),
                category=body.get("category"),
                depth=body.get("depth"),
                index_direction=body.get("index_direction"),
                src=sig.get("src"),
                body_src=sig.get("body_src"),
                init_src=sig.get("init_src"),
                body_is_block=sig.get("body_is_block"),
            )
        )
        ...
        if tdesc.startswith("mapping"):
            ...
            synth.append(
                IRFunction(
                    ...
                    external_calls=[],
                    events_emitted=[],
                    storage_reads=[IRStorageTouch(var=var_name, key=None)],
                    storage_writes=[],
                    synthetic=True,
                )
            )
...
def build(
    ...
    for f in functions:
        ...
        ir_funcs.append(
            IRFunction(
                ...
                storage_reads=[IRStorageTouch(**d) for d in eff.get("storage_reads", [])],
                storage_writes=[IRStorageTouch(**d) for d in eff.get("storage_writes", [])],
                src=f.get("src"),
                body_src=f.get("body_src"),
                return_srcs=f.get("return_srcs") or [],
                length_effects=[IRLengthEffect(**d) for d in eff.get("length_effects", [])],
                storage_aliases=[IRStorageAlias(**d) for d in eff.get("storage_aliases", [])],
    ...
    s_structs = [IRStruct(**d) for d in (state or {}).get("structs", [])]
    ir_state = IRState(variables=s_vars, mappings=s_maps, structs=s_structs)
