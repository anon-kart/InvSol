...
from .extractor_effects import summarize_loop_body
from .loop_roles import annotate_loop_nesting, source_offset
...
def _loop_sig(
    ...
    nesting: Dict[int, Dict[str, Any]] | None = None,
) -> Dict[str, Any]:
    ...
    body_node = loop.get("body") or {}
    sig["src"] = loop.get("src") or ""
    sig["body_src"] = body_node.get("src") or ""
    sig["body_is_block"] = body_node.get("nodeType") == "Block"

    init_node = loop.get("initializationExpression") or {}
    sig["init_src"] = init_node.get("src") or ""

    nest = (nesting or {}).get(loop.get("id")) or {}
    sig["body_summary"] = summarize_loop_body(
        loop,
        state_index,
        depth=int(nest.get("depth", 0)),
        has_inner_loop=nest.get("has_inner_loop"),
    )
...
def extract_loops(norm_ast: Dict[str, Any], state_index: Dict[str, str]) -> List[Dict[str, Any]]:
    ...
    for contract in norm_ast.get("contracts", []):
        nesting = annotate_loop_nesting(contract)
        per_function: Dict[str, int] = {}

        found = (
            list(find_by_type(contract, "ForStatement"))
            + list(find_by_type(contract, "WhileStatement"))
            + list(find_by_type(contract, "DoWhileStatement"))
        )
        found.sort(key=source_offset)

        for lp in found:
            sig = _loop_sig(lp, state_index, nesting)
            fn = find_parent_function(contract, lp)
            fn_name = (fn or {}).get("name") or (fn or {}).get("kind") or "unknown"

            ordinal = per_function.get(fn_name, 0)
            per_function[fn_name] = ordinal + 1

            nest = nesting.get(lp.get("id")) or {}
            contract_name = contract.get("name") or ""

            loops.append({
                "contract": contract_name,
                "function": fn_name,
                "loop_id": f"{contract_name}.{fn_name}#loop{ordinal}",
                "loop_index": ordinal,
                "signature": sig,
                "node_id": lp.get("id"),
                "parent_node_id": nest.get("parent_id"),
            })
    return loops
