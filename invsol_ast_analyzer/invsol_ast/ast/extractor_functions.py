...
def extract_functions(norm_ast: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for contract in norm_ast.get("contracts", []):
        ...
        for fn in find_by_type(contract, "FunctionDefinition"):
            ...
            returns_srcs = [
                r.get("src")
                for r in find_by_type(fn, "Return")
            ...
            funcs.append({
                ...
                # Source spans, needed to place state probes at the function
                # boundary. A body span alone is not enough because an early
                # return would skip an exit probe placed at the end of it.
                "src": fn.get("src"),
                "body_src": body.get("src"),
                "return_srcs": returns_srcs,
            })
