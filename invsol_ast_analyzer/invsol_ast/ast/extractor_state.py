...
def extract_structs(norm_ast: Dict[str, Any]) -> List[Dict[str, Any]]:
    """
    Struct definitions and their fields.

    A struct has no single value to record, so the only way to observe one is
    field by field. Section 3.4.1 flattens a struct u with fields age and
    balance into u_age and u_balance, which needs the field list.
    """
    ...
    for contract in norm_ast.get("contracts", []):
        ...
        for definition in find_by_type(contract, "StructDefinition"):
            name = definition.get("name") or ""
            if not name or (cname, name) in seen:
                continue
            seen.add((cname, name))

            fields: List[Dict[str, str]] = []
            for member in definition.get("members") or []:
                field_name = member.get("name") or ""
                if not field_name:
                    continue
                fields.append({"name": field_name, "type": _type_string(member)})

            out.append({"contract": cname, "name": name, "fields": fields})
...
def extract_state(norm_ast: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        "variables": variables,
        "mappings": mappings,
        "structs": extract_structs(norm_ast),
    }
